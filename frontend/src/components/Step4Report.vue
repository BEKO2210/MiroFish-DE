  ...
  <div class="report-panel">
    ...
    <div class="main-split-layout">
      ...
      <div class="left-panel report-style" ref="leftPanel">
        <div v-if="reportOutline" class="report-content-wrapper">
          ...
          <div class="report-header-block">
            <div class="report-meta">
              <span class="report-tag">Vorhersagebericht</span>
            ...
            >
              ...
              <div class="section-body" v-show="!collapsedSections.has(idx)">
                ...
                <div v-else-if="currentSectionIndex === idx + 1" class="loading-state">
                  ...
                  <span class="loading-text">{{ section.title }} wird generiert...</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div v-if="!reportOutline" class="waiting-placeholder">
          ...
          <span class="waiting-text">Warten auf Berichts-Agent...</span>
        </div>
      </div>
      ...
      <div class="right-panel" ref="rightPanel">
        ...
        <div class="workflow-overview" v-if="agentLogs.length > 0 || reportOutline">
          ...
          <!-- Nächster Schritt Button - nach Abschluss anzeigen -->
          <button v-if="isComplete" class="next-step-btn" @click="goToInteraction">
            <span>Zur Tiefeninteraktion</span>
        ...
        <div class="workflow-timeline">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <template v-if="log.action === 'report_start'">
                    ...
                    <div class="info-row" v-if="log.details?.simulation_requirement">
                      <span class="info-key">Anforderung</span>
                  ...
                  <template v-if="log.action === 'planning_complete'">
                    ...
                    <div class="outline-badge" v-if="log.details?.outline">
                      {{ log.details.outline.sections?.length || 0 }} Abschnitte geplant
                    </div>
                  </template>
                  ...
                  <!-- Abschnittsinhalt generiert (Inhalt erstellt, aber Abschnitt evtl. noch nicht abgeschlossen) -->
                  ...
                  <!-- Abschnitt abgeschlossen (Abschnittsgenerierung abgeschlossen) -->
                  ...
                  <template v-if="log.action === 'llm_response'">
                    <div class="llm-meta">
                      ...
                      <span class="meta-tag" :class="{ active: log.details?.has_tool_calls }">
                        Werkzeuge: {{ log.details?.has_tool_calls ? 'Ja' : 'Nein' }}
                      </span>
                      <span class="meta-tag" :class="{ active: log.details?.has_final_answer, 'final-answer': log.details?.has_final_answer }">
                        Endantwort: {{ log.details?.has_final_answer ? 'Ja' : 'Nein' }}
                      </span>
                    </div>
                    <!-- Bei Endantwort spezielle Benachrichtigung anzeigen -->
                    <div v-if="log.details?.has_final_answer" class="final-answer-hint">
                      <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2">
                        <polyline points="20 6 9 17 4 12"></polyline>
                      </svg>
                      <span>Abschnitt "{{ log.section_title }}" Inhalt generiert</span>
                    </div>
                  ...
                  <template v-if="log.action === 'report_complete'">
                    <div class="complete-banner">
                      ...
                      <span>Berichtserstellung abgeschlossen</span>
                    </div>
                  </template>
                </div>
                ...
                <div class="timeline-footer" v-if="log.elapsed_seconds || (log.action === 'tool_call' && log.details?.parameters) || log.action === 'tool_result' || (log.action === 'llm_response' && log.details?.response)">
                  ...
                  <div class="footer-actions">
                    ...
                    <button v-if="log.action === 'tool_call' && log.details?.parameters" class="action-btn" @click.stop="toggleLogExpand(log)">
                      {{ expandedLogs.has(log.timestamp) ? 'Parameter ausblenden' : 'Parameter anzeigen' }}
                    </button>
                    ...
                    <button v-if="log.action === 'tool_result'" class="action-btn" @click.stop="toggleRawResult(log.timestamp, $event)">
                      {{ showRawResult[log.timestamp] ? 'Strukturierte Ansicht' : 'Rohdaten' }}
                    </button>
                    ...
                    <button v-if="log.action === 'llm_response' && log.details?.response" class="action-btn" @click.stop="toggleLogExpand(log)">
                      {{ expandedLogs.has(log.timestamp) ? 'Antwort ausblenden' : 'Antwort anzeigen' }}
                    </button>
                  </div>
                </div>
              </div>
          ...
          <div v-if="agentLogs.length === 0 && !isComplete" class="workflow-empty">
            ...
            <span>Warten auf Agentenaktivität...</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="console-logs">
      <div class="log-header">
        <span class="log-title">KONSOLENAUSGABE</span>
        <span class="log-id">{{ reportId || 'KEIN_BERICHT' }}</span>
      </div>
...
const toggleRawResult = (timestamp, event) => {
  // Position der Schaltfläche relativ zum Viewport speichern
  ...
  // Nach DOM-Aktualisierung Scrollposition anpassen, damit die Schaltfläche an derselben Stelle bleibt
...
const toggleSectionCollapse = (idx) => {
  // Nur abgeschlossene Abschnitte können eingeklappt werden
  if (!generatedSections.value[idx + 1]) return
...
const toolConfig = {
  'insight_forge': {
    name: 'Deep Insight',
    color: 'purple',
    icon: 'lightbulb' // Glühbirnen-Symbol - steht für Einsicht
  },
  'panorama_search': {
    name: 'Panorama Search',
    color: 'blue',
    icon: 'globe' // Globus-Symbol - steht für Panoramasuche
  },
  'interview_agents': {
    name: 'Agent Interview',
    color: 'green',
    icon: 'users' // Benutzer-Symbol - steht für Gespräch
  },
  'quick_search': {
    name: 'Quick Search',
    color: 'orange',
    icon: 'zap' // Blitz-Symbol - steht für Schnelligkeit
  },
  'get_graph_statistics': {
    name: 'Graph Stats',
    color: 'cyan',
    icon: 'chart' // Diagramm-Symbol - steht für Statistik
  },
  'get_entities_by_type': {
    name: 'Entity Query',
    color: 'pink',
    icon: 'database' // Datenbank-Symbol - steht für Entitäten
  }
}
...
const parseInsightForge = (text) => {
  ...
  try {
    // Analysefrage extrahieren
    const queryMatch = text.match(/分析问题:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    // Vorhersageszenario extrahieren
    const reqMatch = text.match(/预测场景:\s*(.+?)(?:\n|$)/)
    if (reqMatch) result.simulationRequirement = reqMatch[1].trim()
    
    // Statistikdaten extrahieren - Format "相关预测事实: X条" abgleichen
    ...
    // Unterfragen extrahieren - vollständig, ohne Mengenbegrenzung
    ...
    // Schlüsselfakten extrahieren - vollständig, ohne Mengenbegrenzung
    ...
    // Kernentitäten extrahieren - vollständig, mit Zusammenfassung und Anzahl zugehöriger Fakten
    const entitySection = text.match(/### 【核心实体】\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
      const entityText = entitySection[1]
      // Entitätsblöcke nach "- **" aufteilen
    ...
    // Beziehungsketten extrahieren - vollständig, ohne Mengenbegrenzung
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Aktuelle gültige Fakten extrahieren - vollständig, ohne Mengenbegrenzung
    ...
    // Historische/abgelaufene Fakten extrahieren - vollständig, ohne Mengenbegrenzung
    ...
    // Beteiligte Entitäten extrahieren - vollständig, ohne Mengenbegrenzung
...
const parseInterview = (text) => {
  ...
  try {
    // Interviewthema extrahieren
    const topicMatch = text.match(/\*\*采访主题:\*\*\s*(.+?)(?:\n|$)/)
    if (topicMatch) result.topic = topicMatch[1].trim()
    
    // Anzahl Interviewter extrahieren (z.B. "5 / 9 simulierte Agenten")
    ...
    // Auswahlbegründung der Interviewpartner extrahieren
    const reasonMatch = text.match(/### 采访对象选择理由\n([\s\S]*?)(?=\n---\n|\n### 采访实录)/)
    if (reasonMatch) {
      result.selectionReason = reasonMatch[1].trim()
    }
    
    // Individuelle Auswahlbegründungen parsen
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // Beispiel: 1. **校友_345（index=1）**：作为武大校友...
        ...
        // Format 2: - 选择Name（index X）：Begründung
        // Beispiel: - 选择家长_601（index 0）：作为家长群体代表...
        if (!headerMatch) {
        ...
        // Beispiel: - **家长_601（index 0）**：作为家长群体代表...
        if (!headerMatch) {
        ...
        if (name) {
          ...
          // Fortsetzungszeile der Begründung (Abschlussabsätze ausschließen)
    ...
    // Jeden Interviewdatensatz extrahieren
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // Titel extrahieren (z.B. "Student", "Lehrkraft" usw.)
      ...
      if (nameRoleMatch) {
        ...
        // Auswahlbegründung dieser Person setzen
        interview.selectionReason = individualReasons[interview.name] || ''
      }
      
      // Kurzbiografie extrahieren
      ...
      if (qMatch) {
        ...
        // Fragen nach Nummerierung aufteilen
        const questions = qText.split(/\n\d+\.\s+/).filter(q => q.trim())
        if (questions.length > 0) {
          // Wenn die erste Frage mit "1." beginnt, Sonderbehandlung erforderlich
      ...
      // Antworten extrahieren - nach Twitter und Reddit getrennt
      ...
      if (answerMatch) {
        ...
        // Plattform-Fallback-Logik (Kompatibilität mit altem Format: nur eine Plattformmarkierung)
        if (!twitterMatch && redditMatch) {
          // Nur Reddit-Antwort vorhanden, nur bei echtem Text als Standard kopieren
          ...
          // Keine Plattformmarkierung (sehr altes Format), Gesamttext als Antwort
          interview.twitterAnswer = answerText
        }
      }
      
      // Schlüsselzitate extrahieren (kompatibel mit verschiedenen Anführungszeichenformaten)
      const quotesMatch = block.match(/\*\*关键引言:\*\*\n([\s\S]*?)(?=\n---|\n####|$)/)
      if (quotesMatch) {
        const quotesText = quotesMatch[1]
        // Bevorzugt > "text" Format abgleichen
        let quoteMatches = quotesText.match(/> "([^"]+)"/g)
        // Fallback: > "text" oder > \u201Ctext\u201D (chinesische Anführungszeichen) abgleichen
        if (!quoteMatches) {
    ...
    // Interviewzusammenfassung extrahieren
...
const parseQuickSearch = (text) => {
  ...
  try {
    // Suchabfrage extrahieren
    const queryMatch = text.match(/搜索查询:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    // Ergebnisanzahl extrahieren
    const countMatch = text.match(/找到\s*(\d+)\s*条/)
    if (countMatch) result.count = parseInt(countMatch[1])
    
    // Relevante Fakten extrahieren - vollständig, ohne Mengenbegrenzung
    ...
    // Kanteninformationen extrahieren (falls vorhanden)
    ...
    // Knoteninformationen extrahieren (falls vorhanden)
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Aktuelle Schlüsselerinnerungen (${props.result.facts.length})`)
        ]),
        h('button', {
          class: ['insight-tab', { active: activeTab.value === 'entities' }],
          onClick: () => { activeTab.value = 'entities' }
        }, [
          h('span', { class: 'tab-label' }, `Kernentitäten (${props.result.entities.length})`)
        ]),
        h('button', {
          class: ['insight-tab', { active: activeTab.value === 'relations' }],
          onClick: () => { activeTab.value = 'relations' }
        }, [
          h('span', { class: 'tab-label' }, `Beziehungsketten (${props.result.relations.length})`)
        ]),
        props.result.subQueries.length > 0 && h('button', {
          ...
          h('span', { class: 'tab-label' }, `Unterfragen (${props.result.subQueries.length})`)
        ])
      ]),
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Neueste Schlüsselfakten aus dem zeitlichen Gedächtnis'),
            h('span', { class: 'panel-count' }, `Gesamt: ${props.result.facts.length} Einträge`)
          ]),
          ...
          }, expandedFacts.value ? `Einklappen ▲` : `Alle ${props.result.facts.length} Einträge anzeigen ▼`)
        ]),
        ...
        activeTab.value === 'entities' && props.result.entities.length > 0 && h('div', { class: 'entities-panel' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `Gesamt: ${props.result.entities.length} Stück`)
          ]),
          h('div', { class: 'entities-grid' },
            (expandedEntities.value ? props.result.entities : props.result.entities.slice(0, 12)).map((entity, i) => 
              h('div', { class: 'entity-tag', key: i, title: entity.summary || '' }, [
                ...
                entity.relatedFactsCount > 0 && h('span', { class: 'entity-fact-count' }, `${entity.relatedFactsCount} Einträge`)
              ])
            )
          ),
          ...
          }, expandedEntities.value ? `Einklappen ▲` : `Alle ${props.result.entities.length} Stück anzeigen ▼`)
        ]),
        ...
        activeTab.value === 'relations' && props.result.relations.length > 0 && h('div', { class: 'relations-panel' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `Gesamt: ${props.result.relations.length} Einträge`)
          ]),
          ...
          }, expandedRelations.value ? `Einklappen ▲` : `Alle ${props.result.relations.length} Einträge anzeigen ▼`)
        ]),
        
        // Sub-queries Tab
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Durch Drift-Abfrage generierte Analyse-Unterfragen'),
            h('span', { class: 'panel-count' }, `Gesamt: ${props.result.subQueries.length} Stück`)
          ]),
        ...
        // Empty state
        activeTab.value === 'facts' && props.result.facts.length === 0 && h('div', { class: 'empty-state' }, 'Keine aktuellen Schlüsselerinnerungen vorhanden'),
        activeTab.value === 'entities' && props.result.entities.length === 0 && h('div', { class: 'empty-state' }, 'Keine Kernentitäten vorhanden'),
        activeTab.value === 'relations' && props.result.relations.length === 0 && h('div', { class: 'empty-state' }, 'Keine Beziehungsketten vorhanden')
      ])
    ])
  }
}
  ...
  setup(props) {
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-header' }, [
        h('div', { class: 'header-main' }, [
          h('div', { class: 'header-title' }, 'Panoramasuche'),
          h('div', { class: 'header-stats' }, [
            h('span', { class: 'stat-item' }, [
              h('span', { class: 'stat-value' }, props.result.stats.nodes),
              h('span', { class: 'stat-label' }, 'Knoten')
            ]),
            h('span', { class: 'stat-divider' }, '/'),
            h('span', { class: 'stat-item' }, [
              h('span', { class: 'stat-value' }, props.result.stats.edges),
              h('span', { class: 'stat-label' }, 'Kanten')
            ]),
            props.resultLength && h('span', { class: 'stat-divider' }, '·'),
            props.resultLength && h('span', { class: 'stat-size' }, formatSize(props.resultLength))
          ])
      ...
      h('div', { class: 'panorama-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Aktuelle gültige Erinnerungen (${props.result.activeFacts.length})`)
        ]),
        h('button', {
          class: ['panorama-tab', { active: activeTab.value === 'historical' }],
          onClick: () => { activeTab.value = 'historical' }
        }, [
          h('span', { class: 'tab-label' }, `Historische Erinnerungen (${props.result.historicalFacts.length})`)
        ]),
        h('button', {
          class: ['panorama-tab', { active: activeTab.value === 'entities' }],
          onClick: () => { activeTab.value = 'entities' }
        }, [
          h('span', { class: 'tab-label' }, `Beteiligte Entitäten (${props.result.entities.length})`)
        ])
      ]),
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Aktuelle gültige Erinnerungen'),
            h('span', { class: 'panel-count' }, `Gesamt: ${props.result.activeFacts.length} Einträge`)
          ]),
          ...
          ) : h('div', { class: 'empty-state' }, 'Keine aktuellen gültigen Erinnerungen vorhanden'),
          props.result.activeFacts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedActive.value = !expandedActive.value }
          }, expandedActive.value ? `Einklappen ▲` : `Alle ${props.result.activeFacts.length} Einträge anzeigen ▼`)
        ]),
        ...
        activeTab.value === 'historical' && h('div', { class: 'facts-panel historical-facts' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `Gesamt: ${props.result.historicalFacts.length} Einträge`)
          ]),
            ...
            (expandedHistorical.value ? props.result.historicalFacts : props.result.historicalFacts.slice(0, INITIAL_SHOW_COUNT)).map((fact, i) => 
              h('div', { class: 'fact-item historical', key: i }, [
                ...
                h('div', { class: 'fact-content' }, [
                  // Versuche Zeitinformationen [time - time] zu extrahieren
                  (() => {
          ...
          ) : h('div', { class: 'empty-state' }, 'Keine historischen Erinnerungen vorhanden'),
          props.result.historicalFacts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedHistorical.value = !expandedHistorical.value }
          }, expandedHistorical.value ? `Einklappen ▲` : `Alle ${props.result.historicalFacts.length} Einträge anzeigen ▼`)
        ]),
        
        // Entities Tab
        activeTab.value === 'entities' && h('div', { class: 'entities-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Beteiligte Entitäten'),
            h('span', { class: 'panel-count' }, `Gesamt: ${props.result.entities.length} Stück`)
          ]),
          ...
          ) : h('div', { class: 'empty-state' }, 'Keine beteiligten Entitäten vorhanden'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Unabhängigen Plattformauswahlstatus für jedes Frage-Antwort-Paar pflegen
    const platformTabs = reactive({}) // { 'agentIdx-qIdx': 'twitter' | 'reddit' }
    
    // Aktuelle Plattformauswahl einer Frage abrufen
    const getPlatformTab = (agentIdx, qIdx) => {
      const key = `${agentIdx}-${qIdx}`
      return platformTabs[key] || 'twitter'
    }
    
    // Plattformauswahl einer Frage festlegen
    ...
    // Prüfen, ob es sich um Plattform-Platzhaltertext handelt
    ...
    // Versuche Antworten nach Fragennummern aufzuteilen
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // Zwei Nummerierungsformate werden unterstützt:
      // 1. "问题X：" oder "问题X:" (chinesisches Format, neues Backend-Format)
      // 2. "1. " oder "\n1. " (Zahl+Punkt, altes Format kompatibel)
      let matches = []
      let match

      // Bevorzugt "问题X：" Format versuchen
      const cnPattern = /(?:^|[\r\n]+)问题(\d+)[：:]\s*/g
      ...
      // Falls nicht gefunden, auf "Zahl." Format zurückfallen
      if (matches.length === 0) {
        const numPattern = /(?:^|[\r\n]+)(\d+)\.\s+/g
      ...
      // Wenn keine oder nur eine Nummerierung gefunden, Gesamttext zurückgeben
      if (matches.length <= 1) {
    ...
    // Antwort zu einer bestimmten Frage abrufen
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // Aufteilung erfolgreich und Index gültig
      if (answers.length > 1 && qIdx < answers.length) {
        return answers[qIdx] || ''
      }

      // Aufteilung fehlgeschlagen: Erste Frage erhält vollständige Antwort, Rest leer
      return qIdx === 0 ? answer : ''
    }
    
    // Prüfen, ob eine Frage Antworten von beiden Plattformen hat (Platzhaltertext filtern)
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // Beide Plattformen haben echte Antworten (kein Platzhaltertext) und unterschiedliche Inhalte
    ...
    return () => h('div', { class: 'interview-display' }, [
        ...
        h('div', { class: 'header-main' }, [
          ...
          h('div', { class: 'header-stats' }, [
            ...
            props.result.totalCount > 0 && h('span', { class: 'stat-item' }, [
              ...
              h('span', { class: 'stat-label' }, 'Gesamt')
            ]),
            props.resultLength && h('span', { class: 'stat-divider' }, '·'),
            props.resultLength && h('span', { class: 'stat-size' }, formatSize(props.resultLength))
          ])
      ...
      props.result.interviews.length > 0 && h('div', { class: 'interview-detail' }, [
        ...
        h('div', { class: 'qa-thread' }, 
          (props.result.interviews[activeIndex.value]?.questions?.length > 0 
            ...
            : [props.result.interviews[activeIndex.value]?.question || 'Keine Frage verfügbar']
          ).map((question, qIdx) => {
            ...
            return h('div', { class: 'qa-pair', key: qIdx }, [
              ...
              h('div', { class: 'qa-question' }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-sender' }, 'Interviewer/in'),
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dualplattform-Umschaltfläche (nur bei echten Antworten von beiden Plattformen anzeigen)
                  ...
                  // Ein-/Ausklappen-Schaltfläche (bei Platzhaltertext nicht anzeigen)
        ...
        props.result.interviews[activeIndex.value]?.quotes?.length > 0 && h('div', { class: 'quotes-section' }, [
          h('div', { class: 'quotes-header' }, 'Schlüsselzitate'),
      ...
      props.result.summary && h('div', { class: 'summary-section' }, [
        h('div', { class: 'summary-header' }, 'Interviewzusammenfassung'),
        h('div', { 
  ...
  setup(props) {
    ...
    return () => h('div', { class: 'quick-search-display' }, [
      ...
      h('div', { class: 'quicksearch-header' }, [
        h('div', { class: 'header-main' }, [
          h('div', { class: 'header-title' }, 'Schnellsuche'),
          h('div', { class: 'header-stats' }, [
            h('span', { class: 'stat-item' }, [
              h('span', { class: 'stat-value' }, props.result.count || props.result.facts.length),
              h('span', { class: 'stat-label' }, 'Ergebnisse')
            ]),
            props.resultLength && h('span', { class: 'stat-divider' }, '·'),
            props.resultLength && h('span', { class: 'stat-size' }, formatSize(props.resultLength))
          ])
        ...
        props.result.query && h('div', { class: 'header-query' }, [
          h('span', { class: 'query-label' }, 'Suche: '),
      ...
      h('div', { class: ['quicksearch-content', { 'no-tabs': !showTabs.value }] }, [
        ...
        ((!showTabs.value) || activeTab.value === 'facts') && h('div', { class: 'facts-panel' }, [
          !showTabs.value && h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Suchergebnisse'),
            h('span', { class: 'panel-count' }, `Gesamt: ${props.result.facts.length} Einträge`)
          ]),
          ...
          ) : h('div', { class: 'empty-state' }, 'Keine relevanten Ergebnisse gefunden'),
          props.result.facts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedFacts.value = !expandedFacts.value }
          }, expandedFacts.value ? `Einklappen ▲` : `Alle ${props.result.facts.length} Einträge anzeigen ▼`)
        ...
        activeTab.value === 'edges' && hasEdges.value && h('div', { class: 'edges-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Zugehörige Beziehungen'),
            h('span', { class: 'panel-count' }, `Gesamt: ${props.result.edges.length} Einträge`)
          ]),
        ...
        activeTab.value === 'nodes' && hasNodes.value && h('div', { class: 'nodes-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Zugehörige Knoten'),
            h('span', { class: 'panel-count' }, `Gesamt: ${props.result.nodes.length} Stück`)
          ]),
...
const statusText = computed(() => {
  if (isComplete.value) return 'Abgeschlossen'
  if (agentLogs.value.length > 0) return 'Wird generiert...'
  return 'Warten'
})
...
// Aktuell aktiver Schritt (für Anzeige oben)
const activeStep = computed(() => {
  const steps = workflowSteps.value
  // Den aktuell aktiven Schritt finden
  const active = steps.find(s => s.status === 'active')
  if (active) return active
  
  // Wenn kein aktiver Schritt, den letzten abgeschlossenen zurückgeben
  const doneSteps = steps.filter(s => s.status === 'done')
  if (doneSteps.length > 0) return doneSteps[doneSteps.length - 1]
  
  // Andernfalls ersten Schritt zurückgeben
...
const workflowSteps = computed(() => {
  ...
  steps.push({
    key: 'planning',
    noLabel: 'PL',
    title: 'Planung / Gliederung',
    status: planningStatus,
    meta: planningStatus === 'active' ? 'IN BEARBEITUNG' : ''
  })
  ...
  sections.forEach((section, i) => {
    ...
    steps.push({
      ...
      title: section.title,
      status,
      meta: status === 'active' ? 'IN BEARBEITUNG' : ''
    })
  })
  ...
  steps.push({
    ...
    meta: completeStatus === 'active' ? 'WIRD ABGESCHLOSSEN' : ''
  })
...
const renderMarkdown = (content) => {
  ...
  // Anfangs-H2-Überschrift entfernen (## xxx), da Abschnittstitel bereits außen angezeigt wird
  let processedContent = content.replace(/^##\s+.+\n+/, '')
  
  // Codeblöcke verarbeiten
  ...
  // Zitatblöcke verarbeiten
  html = html.replace(/^> (.+)$/gm, '<blockquote class="md-quote">$1</blockquote>')
  
  // Listen verarbeiten - Unterlisten unterstützt
  ...
  // Ungeordnete Listen umhüllen
  html = html.replace(/(<li class="md-li"[^>]*>.*?<\/li>\s*)+/g, '<ul class="md-ul">$&</ul>')
  // Geordnete Listen umhüllen
  html = html.replace(/(<li class="md-oli"[^>]*>.*?<\/li>\s*)+/g, '<ol class="md-ol">$&</ol>')

  // Alle Leerzeichen zwischen Listenelementen bereinigen
  html = html.replace(/<\/li>\s+<li/g, '</li><li')
  // Leerzeichen nach Listen-Öffnungstags bereinigen
  html = html.replace(/<ul class="md-ul">\s+/g, '<ul class="md-ul">')
  html = html.replace(/<ol class="md-ol">\s+/g, '<ol class="md-ol">')
  // Leerzeichen vor Listen-Schlusstags bereinigen
  html = html.replace(/\s+<\/ul>/g, '</ul>')
  html = html.replace(/\s+<\/ol>/g, '</ol>')
  
  // Fett- und Kursivschrift verarbeiten
  ...
  // Zeilenumbrüche verarbeiten - Leerzeilen werden Absatztrenner, einzelne Umbrüche werden <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  
  // In Absätze einbetten
  html = '<p class="md-p">' + html + '</p>'
  
  // Leere Absätze bereinigen
  html = html.replace(/<p class="md-p"><\/p>/g, '')
  html = html.replace(/<p class="md-p">(<h[2-5])/g, '$1')
  html = html.replace(/(<\/h[2-5]>)<\/p>/g, '$1')
  html = html.replace(/<p class="md-p">(<ul|<ol|<blockquote|<pre|<hr)/g, '$1')
  html = html.replace(/(<\/ul>|<\/ol>|<\/blockquote>|<\/pre>)<\/p>/g, '$1')
  // <br>-Tags vor und nach Block-Elementen bereinigen
  html = html.replace(/<br>\s*(<ul|<ol|<blockquote)/g, '$1')
  html = html.replace(/(<\/ul>|<\/ol>|<\/blockquote>)\s*<br>/g, '$1')
  // <p><br> direkt vor Block-Elementen bereinigen (durch überzählige Leerzeilen verursacht)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Aufeinanderfolgende <br>-Tags bereinigen
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // <br> vor Absatz-Öffnungstags nach Block-Elementen bereinigen
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Nummerierung nicht-aufeinanderfolgender geordneter Listen korrigieren: Bei durch Absätze getrennten Einzel-<ol> Nummerierung beibehalten
...
const getActionLabel = (action) => {
  const labels = {
    'report_start': 'Bericht gestartet',
    'planning_start': 'Planung',
    'planning_complete': 'Plan abgeschlossen',
    'section_start': 'Abschnitt gestartet',
    'section_content': 'Inhalt bereit',
    'section_complete': 'Abschnitt fertig',
    'tool_call': 'Werkzeugaufruf',
    'tool_result': 'Werkzeugergebnis',
    'llm_response': 'LLM-Antwort',
    'report_complete': 'Abgeschlossen'
  }
  return labels[action] || action
}

const getLogLevelClass = (log) => {
  if (log.includes('ERROR') || log.includes('Fehler')) return 'error'
  if (log.includes('WARNING') || log.includes('Warnung')) return 'warning'
  // INFO verwendet Standardfarbe, nicht als Erfolg markieren
  return ''
}
    ...
    if (res.success && res.data) {
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          // section_complete - Abschnittsgenerierung abgeschlossen
          if (log.action === 'section_complete') {
            if (log.details?.content) {
              generatedSections.value[log.section_index] = log.details.content
              // Gerade generierten Abschnitt automatisch aufklappen
          ...
          if (log.action === 'report_complete') {
            ...
            currentSectionIndex.value = null  // Sicherstellen, dass der Ladezustand gelöscht wird
            emit('update-status', 'completed')
            stopPolling()
            // Scrolllogik einheitlich im nextTick nach der Schleife verarbeiten
          }
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // Bei abgeschlossener Aufgabe nach oben scrollen; sonst nach unten zum neuesten Log
            if (isComplete.value) {
...
// Endantwort-Inhalt extrahieren - Abschnittsinhalte aus LLM-Antwort extrahieren
const extractFinalContent = (response) => {
  if (!response) return null
  
  // Versuche Inhalt innerhalb des <final_answer>-Tags zu extrahieren
  ...
  // Versuche Inhalt nach Final Answer: zu finden (verschiedene Formate)
  ...
  // Versuche Inhalt nach 最终答案: zu finden
  const chineseFinalMatch = response.match(/最终答案[:：]\s*\n*([\s\S]*)$/i)
  if (chineseFinalMatch) {
    return chineseFinalMatch[1].trim()
  }
  
  // Wenn mit ## oder # oder > beginnend, möglicherweise direkter Markdown-Inhalt
  const trimmedResponse = response.trim()
  if (trimmedResponse.match(/^[#>]/)) {
    return trimmedResponse
  }
  
  // Bei langem Inhalt mit Markdown-Format versuchen, Denkprozess zu entfernen und zurückzugeben
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // Denkprozess mit Thought:-Anfang entfernen
...
onMounted(() => {
  if (props.reportId) {
    addLog(`Berichts-Agent initialisiert: ${props.reportId}`)
    startPolling()
  }
})
...
.section-number {
  ...
  font-size: 16px;
  color: #9CA3AF; /* Dunkelgrau, ändert sich nicht mit dem Status */
  font-weight: 500;
}
